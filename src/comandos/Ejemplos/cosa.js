...
module.exports = class cosa extends Comandos {
  constructor() {
    super('', 'Ejemplos', [""]);
  }
...
async run(client, message, args){
  ...
  //Cooldown system by MaikelG//
      ...
      if (!cooldowns.has(cosa.name)) {
        cooldowns.set(cosa.name, new Map());
      }
    
      const now = Date.now();
      const timestamps = cooldowns.get(cosa.name);
      const cooldownAmount = (cosa.cooldown || 15) * 3000;
      ...
      if (timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          try {
            return message.reply(`Necesitas Esperar ${timeLeft.toFixed(1)} Segundos Para Poder Usar El Comando \`${cosa.name}\` `);
